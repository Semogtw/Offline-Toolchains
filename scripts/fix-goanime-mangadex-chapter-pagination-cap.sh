...
request="$(git diff-tree --no-commit-id --name-only --diff-filter=A -r HEAD^ HEAD -- 'triggers/goanime-mangadex-chapter-pagination-cap-fix/*.request' | head -n 1)"
test -n "$request"
...
set +e
flutter test --no-pub test/services/manga/providers/mangadex_manga_provider_test.dart > /tmp/mangadex-cap-red.log 2>&1
red_status=$?
set -e
test "$red_status" -ne 0
grep -Fq 'rejects chapter pagination that exceeds request cap' /tmp/mangadex-cap-red.log
echo '[tdd] RED observed: MangaDex silently truncates chapter pagination at request cap'
...
"""
if text.count(old) != 1:
    raise SystemExit(f'unexpected MangaDex chapter loop header: {text.count(old)}')
text = text.replace(old, new, 1)
old = """      final nextOffset = responseOffset + limit;
      if (nextOffset >= total || rawItems.isEmpty) break;
      offset = nextOffset;
    }

    chapters.sort((left, right) {
"""
new = """      final nextOffset = responseOffset + limit;
      if (nextOffset >= total || rawItems.isEmpty) {
    ...
    if (!paginationComplete) {
      throw const FormatException(
        'MangaDex chapter pagination exceeded the 100-request safety cap.',
      );
    }
...
"""
if text.count(old) != 1:
    raise SystemExit(f'unexpected MangaDex chapter loop tail: {text.count(old)}')
text = text.replace(old, new, 1)
...
flutter analyze --no-pub lib/services/manga/providers/mangadex_manga_provider.dart test/services/manga/providers/mangadex_manga_provider_test.dart
...
git commit -m 'fix(manga): fail closed on MangaDex chapter pagination cap'
published_sha="$(git rev-parse HEAD)"
remote_head="$(git -c http.extraheader="AUTHORIZATION: basic $auth" ls-remote "$remote_url" "refs/heads/$target_branch" | awk '{print $1}')"
test "$remote_head" = "$expected_sha"
git -c http.extraheader="AUTHORIZATION: basic $auth" push -q origin "HEAD:refs/heads/$target_branch"
echo "[mangadex-chapter-cap] published $published_sha"
popd >/dev/null
rm -rf private-source /tmp/mangadex-cap-red.log
