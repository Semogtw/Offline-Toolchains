#!/usr/bin/env python3
"""Validate reusable public metadata for a GoAnime private-source export."""
...
GOANIME_PREFIX = "private-source-goanime-"
...
def verify_receipt(
    ...
    *,
    ...
    workflow_head_sha = payload.get("workflow_head_sha")
    if not isinstance(workflow_head_sha, str) or not SHA_PATTERN.fullmatch(
        workflow_head_sha
    ):
        raise ValueError("workflow_head_sha must be a lowercase 40-character SHA")
    if payload.get("workflow_event") != "workflow_run":
        raise ValueError("workflow_event must be workflow_run")
    ...
    if not parts:
        raise ValueError("receipt must contain at least one encrypted part")
    ...
    if actual_numbers != expected_numbers:
        raise ValueError(
            "encrypted part numbers must be contiguous from 000; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Invalid GoAnime source receipt: {error}", file=sys.stderr)
        return 1
