...
request="$(git diff-tree --no-commit-id --name-only --diff-filter=A -r HEAD^ HEAD -- 'triggers/goanime-manhastro-search-cursor-fix/*.request' | head -n 1)"
test -n "$request"
...
set +e
flutter test --no-pub test/services/manga/providers/manhastro_manga_provider_test.dart > /tmp/manhastro-cursor-red.log 2>&1
red_status=$?
set -e
test "$red_status" -ne 0
grep -Fq 'rejects malformed search page tokens before I/O' /tmp/manhastro-cursor-red.log
echo '[tdd] RED observed: malformed Manhastro cursor falls back to page 1'
...
"""
if text.count(old) != 1:
    raise SystemExit(f'unexpected Manhastro search shape: {text.count(old)}')
...
if text.count(anchor) != 1:
    raise SystemExit(f'unexpected Manhastro positive-int helper shape: {text.count(anchor)}')
text = text.replace(anchor, anchor + helper, 1)
path.write_text(text, encoding='utf-8')
PY

dart format lib/services/manga/providers/manhastro_manga_provider.dart test/services/manga/providers/manhastro_manga_provider_test.dart
dart format --output=none --set-exit-if-changed lib/services/manga/providers/manhastro_manga_provider.dart test/services/manga/providers/manhastro_manga_provider_test.dart
git diff --check
...
git add lib/services/manga/providers/manhastro_manga_provider.dart test/services/manga/providers/manhastro_manga_provider_test.dart
git diff --cached --check
git commit -m 'fix(manga): reject malformed Manhastro search cursor'
published_sha="$(git rev-parse HEAD)"

remote_head="$(git -c http.extraheader="AUTHORIZATION: basic $auth" ls-remote "$remote_url" "refs/heads/$target_branch" | awk '{print $1}')"
test "$remote_head" = "$expected_sha"
git -c http.extraheader="AUTHORIZATION: basic $auth" push -q origin "HEAD:refs/heads/$target_branch"
echo "[manhastro-cursor] published $published_sha"
popd >/dev/null

rm -rf private-source /tmp/manhastro-cursor-red.log
