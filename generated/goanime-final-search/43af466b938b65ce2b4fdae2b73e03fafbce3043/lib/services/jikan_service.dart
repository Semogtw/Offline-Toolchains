...
class JikanService {
  ...
  static T? _getEntityFromCache<T>(Map<int, _CacheEntry<T>> cache, int key) {
  ...
  Future<HomeData> _loadFreshHomeData() async {
    ...
    try {
      // Busca todos os dados SEQUENCIALMENTE com pausa para respeitar rate limit (3/sec)
      final stopwatch = Stopwatch()..start();
      final today = _getCurrentDayOfWeek();

      final seasonAnimes = await getCurrentSeasonAnimes(limit: 25);
      await Future<void>.delayed(const Duration(milliseconds: 350));

      final todaysReleases = await getScheduleForDay(today, limit: 15);
      await Future<void>.delayed(const Duration(milliseconds: 350));

      final topAnimes = await getTopAnimes(limit: 15);
      await Future<void>.delayed(const Duration(milliseconds: 350));

      final actionAnimes = await getAnimesByGenre(
        JikanGenreIds.action,
        limit: 15,
      );
      await Future<void>.delayed(const Duration(milliseconds: 350));

      // Ordena estreias recentes (seasons) pela data mais recente
      final seasonList = seasonAnimes;
      seasonList.sort((a, b) {
      ...
      final romanceAnimes = await getAnimesByGenre(
        JikanGenreIds.romance,
        limit: 15,
      );
      await Future<void>.delayed(const Duration(milliseconds: 350));

      final comedyAnimes = await getAnimesByGenre(
        JikanGenreIds.comedy,
        limit: 15,
      );
      await Future<void>.delayed(const Duration(milliseconds: 350));

      final fantasyAnimes = await getAnimesByGenre(
      ...
      return HomeData(
        ...
        topAnimes: topAnimes,
        actionAnimes: actionAnimes,
        romanceAnimes: romanceAnimes,
        comedyAnimes: comedyAnimes,
        fantasyAnimes: fantasyAnimes,
      );
    } catch (e) {
  ...
  Future<JikanAnime?> getAnimeById(int malId) {
    ...
    load = _fetchAnimeById(malId).whenComplete(() {
      if (identical(_animeByIdInFlight[malId], load)) {
        final removed = _animeByIdInFlight.remove(malId);
        if (removed != null && !identical(removed, load)) {
          _animeByIdInFlight[malId] = removed;
        }
      }
    });
  ...
  Future<List<JikanAnimeRelation>> getAnimeRelations(int malId) {
    ...
    load = _fetchAnimeRelations(malId).whenComplete(() {
      if (identical(_animeRelationsInFlight[malId], load)) {
        final removed = _animeRelationsInFlight.remove(malId);
        if (removed != null && !identical(removed, load)) {
          _animeRelationsInFlight[malId] = removed;
        }
      }
    });
  ...
  Future<List<JikanAnime>> searchAnimeMetadata(
    ...
    final cacheKey = _metadataSearchCacheKey(
      normalizedQuery,
      limit,
      availabilityMode,
    );
  ...
  String _metadataSearchCacheKey(
    String query,
    int limit,
